...
serve(async (req) => {
  ...
  try {
    ...
    const webhookSecret = Deno.env.get("STRIPE_WEBHOOK_SECRET_MATCH")!;
    ...
    console.log("╔═══════════════════════════════════════════════════════════╗");
    console.log("║       WEBHOOK MATCH (FSCG) - SUPABASE                     ║");
    console.log("╚═══════════════════════════════════════════════════════════╝");
    ...
    if (event.type === "checkout.session.completed") {
      ...
      // ═══════════════════════════════════════════════════════════════
      // 🛡️ GUARD: Samo "match" sesije — ostale ignorišemo
      // ═══════════════════════════════════════════════════════════════
      const eventType = session.metadata?.event_type;
      if (eventType !== "match") {
        console.log(`⏭️ Skipping non-match session (event_type: ${eventType || "undefined"})`);
        return new Response(JSON.stringify({ received: true, skipped: true, reason: "not a match session" }), {
      ...
      // ═══════════════════════════════════════════════════════════════
      // CUSTOMER DATA (metadata fallback za FSCG match karte)
      // ═══════════════════════════════════════════════════════════════
      ...
      let customerPhone = metadata.customer_phone || session.customer_details?.phone || "";
      let customerGender = metadata.customer_gender || customFields?.find((f: any) => f.key === "pol")?.dropdown?.value || "";
      let customerCity = session.customer_details?.address?.city || metadata.customer_city || "";
      const customerAddress = session.customer_details?.address?.line1 || metadata.customer_address || "";
      const customerZip = session.customer_details?.address?.postal_code || metadata.customer_zip || "";

      const customerCountry = session.customer_details?.address?.country || metadata.customer_country || "";
      ...
      if (seatsioKey && eventId && seats.length > 0) {
        const bookingResult = await bookSeatsInSeatsio(
          eventId,
          seats,
          generateOrderNumber(session.id),
          holdToken,
          seatsioKey
        );
        ...
        if (!bookingResult.success) {
          console.error("❌ SEATS.IO BOOKING FAILED - NE KREIRAM KARTE!");
          console.error("❌ Razlog:", bookingResult.message);
          console.error("❌ Error:", JSON.stringify(bookingResult.error));

          // Ažuriraj ProcessedWebhooks sa greškom
          await supabase
            .from("ProcessedWebhooks")
            .update({
              status: "failed_booking",
              errorMessage: `Seats.io booking failed: ${bookingResult.message}`
            })
            .eq("stripeEventId", event.id);

          // Vrati 200 da Stripe ne retry-uje (plaćanje je prošlo, treba ručna intervencija)
      ...
      const viewUrl = `${appBaseUrl}/fscg-karta?sessionId=${session.id}`;
      ...
      // ═══════════════════════════════════════════════════════════════
      // RECONSTRUCT DOCUMENT IMAGE URL FROM CHUNKED METADATA
      // ═══════════════════════════════════════════════════════════════
      let documentImageUrl = "";
      const docUrlChunks = parseInt(metadata.document_image_url_chunks || "0");
      if (docUrlChunks > 0) {
        for (let i = 0; i < docUrlChunks; i++) {
          documentImageUrl += metadata[`doc_url_${i}`] || "";
        }
      }
      console.log("WEBHOOK MATCH (FSCG) - Document type:", metadata.document_type || "none");
      console.log("WEBHOOK MATCH (FSCG) - Document number:", metadata.document_number || "none");
      console.log("WEBHOOK MATCH (FSCG) - Document image URL chunks:", docUrlChunks);
      if (documentImageUrl) {
        console.log("WEBHOOK MATCH (FSCG) - Document image URL reconstructed:", documentImageUrl.substring(0, 80) + "...");
      }
      ...
      for (const seat of seats) {
        ...
        ticketDataArray.push({
          ...
          doc_surname: metadata.ocr_surname || "",
          doc_given_names: metadata.ocr_given_names || "",
          doc_sex: metadata.ocr_sex || "",
          doc_expiry_date: metadata.doc_expiry_date || "",
        });
      }
      ...
      // ═══════════════════════════════════════════════════════════════
      try {
        const seatReservationSessionId = metadata.seat_reservation_session_id;
        if (seatReservationSessionId) {
          ...
          if (ticketIds.length > 0) {
            const { error: confirmError } = await supabase.rpc("confirm_seats", {
              p_session_id: seatReservationSessionId,
              p_ticket_ids: ticketIds,
            });
            if (confirmError) {
              console.log("⚠️ confirm_seats (non-critical):", confirmError.message);
            } else {
              console.log("✅ Stadium seats confirmed for session:", seatReservationSessionId);
            }
          }
        }
      } catch (seatErr: any) {
      ...
      if (wixBackupEnabled && createdTickets.length > 0) {
        backupToWix(createdTickets, session.id, "stripe-webhook-match").catch(e => {
      ...
      // ═══════════════════════════════════════════════════════════════
      // ✅ Ažuriraj status na "completed" (upisan je kao "processing" na početku)
      // ═══════════════════════════════════════════════════════════════
      await supabase
        .from("ProcessedWebhooks")
      ...
      console.log("╔═══════════════════════════════════════════════════════════╗");
      console.log("║       ✅ WEBHOOK MATCH (FSCG) COMPLETE                     ║");
      console.log("╚═══════════════════════════════════════════════════════════╝");
      console.log("Summary:");
    ...
    if (event.type === "checkout.session.expired") {
      ...
      // Release stadium seats if reservation session exists
      const seatResId = expiredMeta.seat_reservation_session_id;
      if (seatResId) {
        try {
          const { error: releaseError } = await supabase.rpc("release_seats", {
            p_session_id: seatResId,
          });
          if (releaseError) {
            console.log("⚠️ release_seats error:", releaseError.message);
          } else {
            console.log("✅ Released reserved seats for expired session:", seatResId);
          }
        } catch (e: any) {
      ...
      // Release seats.io hold if exists
      const holdToken = expiredMeta.hold_token;
      if (holdToken && seatsioKey) {
        try {
          await fetch(`https://api-eu.seatsio.net/hold-tokens/${holdToken}`, {
            method: "DELETE",
            headers: {
              Authorization: `Basic ${btoa(seatsioKey + ":")}`,
            },
          });
          console.log("✅ Released seats.io hold token:", holdToken);
        } catch (e: any) {
          console.log("⚠️ seats.io hold release skipped:", e?.message);
        }
      }
    }
