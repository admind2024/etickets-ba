...
const FscgSeatSelection = () => {
  ...
  // FSCG: Skip cart modal — go directly to custom checkout form
  const goDirectToCheckout = async () => {
    if (selectedSeats.length === 0) return;
    ...
    try {
      ...
      sessionStorage.setItem(`fscgCheckoutData_${slug}`, JSON.stringify(checkoutState));
      sessionStorage.setItem(`fscgCheckoutStart_${slug}`, Date.now().toString());
      navigate(`/checkout/fscg/${slug}`);
    } catch (error: any) {
  ...
  const proceedToPayment = async () => {
    // Redirect to goDirectToCheckout (kept for backward compatibility if cart modal is still used somewhere)
    await goDirectToCheckout();
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {isMatch && homeTeam && awayTeam ? (
        /* ═══ SPORT MATCH HEADER (compact on mobile) ═══ */
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
          <div className="w-full max-w-[1200px] mx-auto px-3 py-3 md:px-4 md:py-8">
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-1.5 text-white/60 hover:text-white mb-2 md:mb-5 transition-colors text-xs md:text-sm"
            >
              <ArrowLeft className="w-3.5 h-3.5 md:w-4 md:h-4" />
              <span>{t.back}</span>
            </button>

            {/* VS Layout - compact on mobile */}
            <div className="flex items-center justify-center gap-3 md:gap-8 mb-2 md:mb-6">
              {/* Home Team */}
              <div className="flex flex-col items-center text-center flex-1 min-w-0">
                {homeTeam.logo ? (
                  <img src={homeTeam.logo} alt={homeTeam.name} className="w-10 h-10 md:w-24 md:h-24 object-contain mb-1 md:mb-3" />
                ) : (
                  <div className="w-10 h-10 md:w-24 md:h-24 rounded-full bg-white/10 flex items-center justify-center mb-1 md:mb-3">
                    <span className="text-base md:text-3xl font-black text-white/40">
                      {homeTeam.short_name?.charAt(0) || homeTeam.name.charAt(0)}
                    </span>
                  </div>
                )}
                <h2 className="text-white font-bold text-xs md:text-lg leading-tight truncate max-w-full">
              ...
              <div className="flex-shrink-0">
                <span className="text-lg md:text-4xl font-black text-white/50">VS</span>
              </div>

              {/* Away Team */}
              <div className="flex flex-col items-center text-center flex-1 min-w-0">
                {awayTeam.logo ? (
                  <img src={awayTeam.logo} alt={awayTeam.name} className="w-10 h-10 md:w-24 md:h-24 object-contain mb-1 md:mb-3" />
                ) : (
                  <div className="w-10 h-10 md:w-24 md:h-24 rounded-full bg-white/10 flex items-center justify-center mb-1 md:mb-3">
                    <span className="text-base md:text-3xl font-black text-white/40">
                      {awayTeam.short_name?.charAt(0) || awayTeam.name.charAt(0)}
                    </span>
                  </div>
                )}
                <h2 className="text-white font-bold text-xs md:text-lg leading-tight truncate max-w-full">
            ...
            {/* Date, Time, Venue info bar - single line on mobile */}
            <div className="flex items-center justify-center gap-2 md:gap-6 flex-wrap text-white/60 text-[11px] md:text-sm">
      ...
      <div className="w-full max-w-[1200px] mx-auto px-3">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            >
              {isSelectionInvalid
                ? (language === "en" ? "⚠️ Deselect to avoid leaving a single empty seat" : "⚠️ Deselektuj da ne ostane jedno prazno sjedište")
          ...
          <button
            onClick={goDirectToCheckout}
            disabled={selectedSeats.length === 0 || isProcessing || isSelectionInvalid}
            className={`relative px-5 py-2.5 rounded-lg text-sm font-bold text-white transition-all disabled:bg-gray-300 disabled:cursor-not-allowed ${
              selectedSeats.length > 0 && !isProcessing && !isSelectionInvalid ? "cart-shimmer cart-animated" : "bg-gray-300"
            }`}
          >
            {isProcessing ? "Učitavanje..." : "Kupi"}
          </button>
        </div>
      </div>
      ...
      {/* ═══ PLAYERS SECTION ═══ */}
      {isMatch && (homePlayers.length > 0 || awayPlayers.length > 0) && (
        <div className="w-full max-w-[1200px] mx-auto px-3 py-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4 text-center">
            {language === "en" ? "Squads" : "Sastavi"}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Home Team Players */}
            {homePlayers.length > 0 && homeTeam && (
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-4 py-3 flex items-center gap-3">
                  ...
                  <h3 className="text-white font-bold text-sm">{homeTeam.name}</h3>
                </div>
                    ...
                    <div key={player.id} className="flex items-center gap-3 px-4 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {player.is_captain && (
                            <span className="text-[10px] bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded font-bold flex-shrink-0">C</span>
                          )}
                        </div>
                      </div>
                      {player.position && (
            ...
            {/* Away Team Players */}
            {awayPlayers.length > 0 && awayTeam && (
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-4 py-3 flex items-center gap-3">
                  {awayTeam.logo && (
                    <img src={awayTeam.logo} alt={awayTeam.name} className="w-8 h-8 object-contain" />
                  )}
                  <h3 className="text-white font-bold text-sm">{awayTeam.name}</h3>
                </div>
                <div className="divide-y divide-gray-100">
...
export default FscgSeatSelection;
